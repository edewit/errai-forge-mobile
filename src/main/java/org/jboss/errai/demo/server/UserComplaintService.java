...
public class UserComplaintService {
  ...
  public List<UserComplaint> search(String query) {
    return em.createQuery("SELECT c FROM UserComplaint c WHERE c.name like :query", UserComplaint.class)
        .setParameter("query", query + "%").getResultList();
  }

  public List<UserComplaint> findAll(int start, int limit) {
    return em.createQuery("SELECT c FROM UserComplaint c", UserComplaint.class)
